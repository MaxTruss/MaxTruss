...
int file_readPrivate(char *szFileName, map<int, list<int> > &mpPrivate, int *piMaxPid)
{
    ...
    while (fgets(szBuffer, ONE_LINE_BUFFER - 1, fp) > 0)
    {
        ...
        {
            /* root node */
            if (!lstNeighbor.empty())
            {
                /* save */
                map<int, list<int> >::iterator itmpNode;
                itmpNode = mpPrivate.find(iNode);
                /* empty */
                DEBUG_ASSERT(itmpNode == mpPrivate.end());

                mpPrivate[iNode].clear();
                mpPrivate[iNode].swap(lstNeighbor);
        ...
        else
        {
            ...
            if (iNode == iTempX)
            {
                lstNeighbor.push_back(iTempY);
                //printf("READ neighbor: %d\n", iTempY);
            }
            else if (iNode == iTempY)
            {
                lstNeighbor.push_back(iTempX);
                //printf("READ neighbor: %d\n", iTempX);
            }
            ++iLocalCnt;
        }
...
int file_saveTrussness(myG &oMpG, char *pcFile)
{
    ...
    //printf("DEBUG enter save trussness size: %d\n", oMpG.m_iMaxEId);
    //for (iEid = 1; iEid <= oMpG.m_iMaxEId; ++iEid)
    for (iEid = oMpG.m_iMaxEId; iEid > 0; --iEid)
    {
        //printf("DEBUG eid: %d\n", iEid);
        pstTpNode = oMpG.findNode(iEid);
        DEBUG_ASSERT(NULL != pstTpNode);
        //printf("DEBUG eid: %d %d\n", iEid, pstTpNode->eid);

        if (iEid != pstTpNode->eid)
        {
            continue;
        }
    ...
    //printf("DEBUG construct detruss\n");
...
int file_readInsE(char *pcFile, vector<pair<int, int> > &vInsE)
{
    ...
    while (fgets(szBuffer, ONE_LINE_BUFFER - 1, fp) > 0)
    {
        sscanf(szBuffer, "%d %d", &x, &y);
        vInsE.push_back(pair<int, int>(x + g_iOffset, y + g_iOffset));
    }
    fclose(fp);
    fp = NULL;
    return 0;
